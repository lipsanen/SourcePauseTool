...
ConVar tas_anglespeed("tas_anglespeed", "5", FCVAR_CHEAT, "Determines the speed of angle changes when using tas_aim\n");
...
#define RNMX (1.0 - EPS)
	...
	struct ViewState
	{
		...
		{
			// Duplicate these variables from the struct so we can reuse the same code for TAS strafing
			float localTargetYaw;
			int localFramesLeft;
			bool localSet;
			current[YAW] = newYaw;

			if (!set && tas_strafe.GetBool() && tas_strafe_vectorial.GetInt() && tas_strafe_version.GetInt() >= 4)
			{
				// Set the locals to TAS strafing values
				localSet = true;
				localFramesLeft = 0;
				localTargetYaw = utils::NormalizeDeg(tas_strafe_yaw.GetFloat());
			}
			else
			{
				// Otherwise copy the normal values
				localFramesLeft = ticksLeft;
				localSet = set;
				localTargetYaw = target[YAW];
			}

			if(!localSet)
				return newYaw;

			float anglespeed = tas_anglespeed.GetFloat();
			float diff = utils::NormalizeDeg(localTargetYaw - current[YAW]);

			if (!timedChange)
			{
				if (diff < anglespeed)
				{
					current[YAW] = localTargetYaw;
				}
				else
				{
					current[YAW] = utils::NormalizeDeg(localTargetYaw + std::copysignf(anglespeed, diff));
				}
			}
			else if(localFramesLeft == 1)
			{
				current[YAW] = localTargetYaw;
			}
			else
			{
				current[YAW] = utils::NormalizeDeg(current[YAW] + diff / ticksLeft);
			}
		...
		float CalculateNewPitch(float newPitch)
		{
			...
			float anglespeed = tas_anglespeed.GetFloat();
			float diff = target[YAW] - current[PITCH];
			...
			if (!timedChange)
			{
				...
				{
					current[PITCH] = current[PITCH] + std::copysignf(anglespeed, diff);
				}
			}
			else if (ticksLeft == 1)
			{
			...
			{
				current[PITCH] = current[PITCH] + diff / ticksLeft;
			}
	...
	bool UpdateView(float& pitch, float& yaw)
	{
...
#define PUNCH_SPRING_CONSTANT 65.0f // bigger number increases the speed at which the view corrects
	...
	CON_COMMAND(tas_aim_reset, "Resets aim target")
	{
	...
	CON_COMMAND(tas_aim, "boom, headshot *music*")
	{
		...
		else
		{
			...
			g_viewState.ticksLeft = std::min(1, frames);
		}
	}
