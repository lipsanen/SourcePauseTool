...
CON_COMMAND(tas_record, "Record gameplay to .srctas script.")
{
...
CON_COMMAND(tas_record_save, "Save recording to .srctas.")
{
	if (args.ArgC() <= 1)
	{
...
CON_COMMAND(tas_record_stop, "Stop recording gameplay.")
{
...
void TASRecordFeature::LoadFeature()
{
	if (ProcessMovementPre_Signal.Works && FrameSignal.Works && ORIG_CCommandBuffer__DequeueNextCommand)
	{
		...
		InitCommand(tas_record);
		InitCommand(tas_record_save);
