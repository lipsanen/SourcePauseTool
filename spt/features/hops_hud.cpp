...
{
	bool result = spt_hud.AddHudCallback(HudCallback("hops", std::bind(&HopsHud::DrawHopHud, this), std::bind(&HopsHud::ShouldDraw, this), false));
...
#define DrawValue(fmt, value) swprintf_s(buffer, BUFFER_SIZE, fmt, value); \
		DrawBuffer();
	...
	if(y_spt_hud_ljstats.GetBool())
	{
		...
		for (auto& segment : ljstats::lastJump.segments) {
			...
			float weight = segment.ticks * 0.015f;

			swprintf_s(buffer,
				BUFFER_SIZE,
				L"%d %7.3f %7.3f %7.3f %7.3f%%",
				i,
