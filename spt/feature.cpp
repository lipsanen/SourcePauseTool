...
void Feature::AddOffsetHook(ModuleEnum moduleEnum, int offset, const char* patternName, void** origPtr, void* functionHook)
{
	auto& mhd = moduleHookData[static_cast<int>(moduleEnum)];
...
void ModuleHookData::HookModule(const std::wstring& moduleName)
{
	...
	{
		hooks.emplace_back(MemUtils::find_unique_sequence_async(*pattern.origPtr, moduleStart, moduleSize, pattern.patternArr, pattern.patternArr + pattern.size));
	}
