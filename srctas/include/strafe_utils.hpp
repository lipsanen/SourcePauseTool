...
namespace srctas
{
	...
	const double SAFEGUARD_THETA_DIFFERENCE_RAD = M_PI / 65536;
	...
	template<typename T1, std::size_t size = 3>
	inline void VecAdd(const T1& a, const T1& b, T1& c)
	{
		for (std::size_t i = 0; i < size; ++i)
			c[i] = a[i] + b[i];
	}

	template<typename T1, typename T1, std::size_t size = 3>
	inline void VecSubtract(const T1& a, T1& c)
	{
		for (std::size_t i = 0; i < size; ++i)
			c[i] = a[i] - b[i];
	}
	...
	template<typename T1, std::size_t size = 3>
	inline double DotProduct(const T1& a, const T1& b)
	{
		double result = 0.0;
		for (std::size_t i = 0; i < size; ++i)
			result += a[i] * b[i];
	...
	template<typename T1, std::size_t size = 3>
	inline double Distance(const T1& a, const T1& b)
	{
	...
	inline void CrossProduct(const T1& a, const T1& b, T1& out)
	{
